...
public class RelectureService {
     ...
     * RG5 révisée : l'exercice ne passe au statut NOTE que lorsque LES DEUX
     * relectures sont rendues. Après la première, il reste en attente et sa note est
     * consultable mais marquée provisoire (RG18).
     *
    ...
    public RelectureResponse rendre(Long relectureId, RendreRelectureRequest requete) {
        ...
        relectureRepository.flush(); // pour que le comptage ci-dessous voie cette relecture

        // RG5 révisée : l'exercice n'est noté qu'une fois les deux relectures rendues.
        if (relectureRepository.countByExerciceIdAndRendueAtIsNotNull(exercice.getId())
                >= AssignationRelecteurService.RELECTEURS_REQUIS) {
            exercice.marquerNote();
        }
